...
<template>
  <div class="p-6 bg-gray-50 shadow-md rounded-md space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <LabeledInput label="생산실적코드" v-model="props.detail.prdr_code" :readonly="true" />
      <LabeledInput label="제품명" v-model="props.detail.prod_code" :readonly="true" />
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <LabeledInput label="작업지시코드" v-model="props.detail.work_order_code" :readonly="true" />
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <LabeledInput label="생산수량" v-model="props.detail.production_qtt" :readonly="true" />
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <LabeledInput label="달성률" v-model="props.detail.peform_rate" :readonly="true" />
      <LabeledTextarea label="비고" v-model="props.detail.note" readonly />
    </div>
  </div>
</template>
