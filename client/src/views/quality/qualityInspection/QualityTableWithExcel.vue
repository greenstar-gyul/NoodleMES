<template>
  ...
  <div class="card mt-6">
    ...
    <!-- DataTable (PrimeVue) -->
    <DataTable v-model:selection="selectedQio" :value="data" :dataKey="dataKey" showGridlines scrollable
      scrollHeight="400px" tableStyle="min-width: 50rem">
      <Column selectionMode="multiple" headerStyle="width: 3rem" />

      <!-- 동적 컬럼 생성 -->
      <Column v-for="item in itemsQio" :key="item" :field="item" :header="mapper[item] ?? item">
        <!-- 날짜포맷변경을 위해 추가한 파트 -->
        <template #body="slotProps">
          <span v-if="['qio_date', 'insp_date'].includes(item)">
...
watch(
  ...
  (newVal) => {
    ...
    if (Array.isArray(newVal) && newVal.length > 0) {
      itemsQio.value = Object.keys(newVal[0]);
    } else {
      itemsQio.value = [];
    }
  },
...
watch(
  ...
  (newVal) => {
    if (newVal.length > 0) {
      itemsQio.value = newVal;
    } else if (Array.isArray(props.data) && props.data.length > 0) {
      itemsQio.value = Object.keys(props.data[0]);
    } else {
      itemsQio.value = [];
    }
  },
...
const emit = defineEmits(['crctQio']);
const crctQio = () => {
  if (selectedQio.value && selectedQio.value.length > 0) {
    emit('crctQio', selectedQio.value);
  } else {
    alert('수정할 행을 선택해주세요.');
  }
};
