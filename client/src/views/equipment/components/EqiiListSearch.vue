...
const fetchSearch = () => {
  ...
  emit('search', searchParams); // 🔥 이벤트 이름 변경
};
...
<template>
  ...
  <div class="p-6 bg-gray-50 shadow-md rounded-md space-y-6">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
      <!-- 점검지시코드 -->
      <SearchText 
        v-model="searchOption.eqii_code" 
        label="점검지시코드" 
        placeholder="점검지시코드를 입력하세요" 
      />
      ...
      <!-- 지시자명 -->
      <SearchText 
        v-model="searchOption.inst_emp_name" 
        label="지시자명" 
        placeholder="지시자명을 입력하세요" 
      />

      <!-- 비고 -->
      <SearchText 
        v-model="searchOption.note" 
        label="비고" 
